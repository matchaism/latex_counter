async function loadCharacterCount() {
  try {
    ...
    // ポスト(ツイート)の用意
    const target = document.getElementById("tweet").href = "https://twitter.com/intent/tweet?text=" + `matchaismの修論の文字数${data.previous_count}->${data.current_count} ${location.href}`;
  } catch (error) {
    console.error(error);
